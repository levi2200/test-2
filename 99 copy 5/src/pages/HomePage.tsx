...
import { analyseTopic, type TopicAnalysis } from '../lib/cinema';
      ...
      {open && (
        <div style={{ padding: '0 1.1rem 1.1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem', flexWrap: 'wrap' }}>
              ...
              <span style={C.chip('rgba(16,185,129,0.15)')}>
                <span style={{ color: '#34d399' }}>SCENE</span>
              </span>
              <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#10b981' }}>{cinema.scene.location}</span>
              <span style={{
                marginLeft: 'auto',
            ...
            <div style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '0.6rem', padding: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.45rem' }}>
                ...
                <span style={C.chip('rgba(59,130,246,0.15)')}>
                  <span style={{ color: '#60a5fa' }}>PROP</span>
                </span>
              </div>
              <p style={{ fontSize: '0.72rem', color: '#71717a', lineHeight: 1.6, margin: 0 }}>
                {cinema.prop.slice(0, 140)}…
              </p>
            </div>
            ...
            <div style={{ background: 'rgba(234,179,8,0.06)', border: '1px solid rgba(234,179,8,0.2)', borderRadius: '0.6rem', padding: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.45rem' }}>
                ...
                <span style={C.chip('rgba(234,179,8,0.15)')}>
                  <span style={{ color: '#facc15' }}>COLOR GRADE</span>
                </span>
              </div>
              <p style={{ fontSize: '0.72rem', color: '#71717a', lineHeight: 1.6, margin: 0 }}>
                {cinema.colorGrade.slice(0, 140)}…
              </p>
            </div>
            ...
            <div style={{ background: 'rgba(168,85,247,0.06)', border: '1px solid rgba(168,85,247,0.2)', borderRadius: '0.6rem', padding: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.45rem' }}>
                ...
                <span style={C.chip('rgba(168,85,247,0.15)')}>
                  <span style={{ color: '#c084fc' }}>LIGHTING</span>
                </span>
              </div>
              <p style={{ fontSize: '0.72rem', color: '#71717a', lineHeight: 1.6, margin: 0 }}>
                {cinema.lighting.slice(0, 140)}…
              </p>
            </div>
          ...
          <p style={{ fontSize: '0.67rem', color: '#3f3f46', margin: 0, textAlign: 'center' }}>
            Scene, expression, prop, grade and lighting all derived from deep semantic topic analysis.
          </p>
        </div>
      )}
    </div>
...
export default function HomePage() {
  ...
  const [cinema, setCinema] = useState<TopicAnalysis | null>(null);
  ...
  const generate = useCallback(() => {
    ...
    setTimeout(() => {
      // Run semantic analysis and store for the debug panel
      const analysis = analyseTopic(topic.trim());
      setCinema(analysis);
      setRegenCount(0);

      // Build the prompt using the new engine
      let result = selectedStyle === 'auto'
        ? buildAutoPrompt(topic.trim(), niche, format)
        : buildMasterPrompt(topic.trim(), selectedStyle, niche, format);
  ...
  /** Regenerate: picks new random camera + lighting variation.
  ...
  function handleRegenerate() {
    ...
    setTimeout(() => {
      // buildMasterPrompt picks new random camera/lighting on each call
      let result = selectedStyle === 'auto'
        ? buildAutoPrompt(topic.trim(), niche, format)
        : buildMasterPrompt(topic.trim(), selectedStyle, niche, format);
      if (safeMode) result = applySafeMode(result);
